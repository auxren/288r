...
| **sens.** | 1 of the 7 | An analog attenuator on the level-detect input (one of the codec's ADC channels). The firmware envelope-follows that signal and compares it against a fixed internal reference, so the knob sets the **AUTO CONTROL** sensitivity by analog gain: fully CCW disables auto triggering (the LED stays dark); raising it lets quieter material trigger. |
| **Preset trimmers** | 36 (4 banks × 9) | The stock firmware's stored **tap positions** for each preset bank, on the printed **0–160** scale. The community firmware does not read them — presets are **saved slots** instead (see [Presets](06-presets-taps-mixers.md)). |
