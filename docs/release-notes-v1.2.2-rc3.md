# 288r community firmware — v1.2.2-rc3 (pre-release for field testing)
...
[#15](https://github.com/auxren/288r/issues/15),
[#21](https://github.com/auxren/288r/issues/21), #10). Supersedes rc1/rc2. **Updating preserves your saved
...
- **The presence LED and auto transport no longer listen to the module's own output**
  (#21, #10): the sens channel's analog pickup also hears loop playback and echoes, which
  kept the LED lit over a silent input and blinded the store-end silence detection. Every
  silence/onset decision is now corroborated by input A's own level.
- **store end auto capture is truly envelope-mastered** (#10): writing continues as long as
  signal is present — no cycle quantization — and silence loops the last phrase, bounded to
  one cycle. Signal present = writing, silence = looping, as designed in the field.
